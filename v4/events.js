function handleClick(event) {
  ...
  if (action === "toggle-memo") return toggleMemoPane();
  if (action === "close-memo") return setMemoOpen(false);
  if (action === "margin") return setMemoOpen(true);
  if (action === "close-margin") return setMemoOpen(false);
  if (action === "jump-node") return jumpToNode(nodeId);
  if (action === "toggle-collapse")
    return closeParagraphMenuThen(() => toggleCollapsed(nodeId));
  if (action === "add-heading")
    return closeParagraphMenuThen(() => revealHeadingAfterRender(nodeId));
  ...
  if (action === "move-up")
    return closeParagraphMenuThen(() => moveNode(nodeId, -1));
  if (action === "move-down")
    return closeParagraphMenuThen(() => moveNode(nodeId, 1));
  if (action === "indent")
    return closeParagraphMenuThen(() => indentNode(nodeId));
  if (action === "outdent")
    return closeParagraphMenuThen(() => outdentNode(nodeId));
  if (action === "duplicate")
    return closeParagraphMenuThen(() => duplicateNode(nodeId));
  if (action === "delete-node")
    return closeParagraphMenuThen(() => deleteNode(nodeId));
  if (action === "move-loose")
    return closeParagraphMenuThen(() => moveToLoose(nodeId));
  if (action === "return-manuscript")
    return closeParagraphMenuThen(() => returnToManuscript(nodeId));
  if (action === "new-root") return addRootNode();
  if (action === "copy-node")
    return closeParagraphMenuThen(() => quickCopyNode(nodeId));
...
function revealHeading(nodeId) {
  const input = document.querySelector(
    `.node-heading[data-node-id="${CSS.escape(nodeId)}"]`,
  );
  if (!input) return;
...
function setMemoOpen(open) {
  if (!state.book) return;
  state.book.view.memoOpen = open;
  render();
  scheduleSave();
  if (open) {
    requestAnimationFrame(() =>
      document.querySelector("#node-note, #next-thread, #global-note")?.focus(),
    );
  }
}
...
function toggleMemoPane() {
  if (!state.book) return;
  setMemoOpen(state.book.view.memoOpen !== true);
}
...
function jumpToNode(nodeId) {
  ...
  state.book.view.collapsedIds = state.book.view.collapsedIds.filter(
    (id) => !pathIds.includes(id),
  );
  state.book.view.selectedNodeId = nodeId;
  state.paragraphMenuNodeId = null;
...
function setAllCollapsed(collapsed) {
  ...
  state.book.view.collapsedIds = collapsed
    ? [
        ...collectCollapsibleIds(state.book.manuscript),
        ...collectCollapsibleIds(state.book.loose),
      ]
    : [];
...
function deleteNode(nodeId) {
  ...
  mutateStructure(
    ...
    () => {
      ...
      if (
        context.container === "manuscript" &&
        state.book.manuscript.length === 0
      ) {
        state.book.manuscript.push(createNode());
      }
      state.book.view.selectedNodeId =
        context.siblings[Math.min(context.index, context.siblings.length - 1)]
          ?.id ??
        state.book.manuscript[0]?.id ??
        null;
      if (state.book.view.hoistedNodeId === nodeId)
        state.book.view.hoistedNodeId = null;
      state.book.view.collapsedIds = state.book.view.collapsedIds.filter(
        (id) => id !== nodeId,
      );
    },
    {
      nodeId: state.book.view.selectedNodeId,
      field: state.book.view.mode === "write" ? "body" : "row",
      offset: 0,
    },
  );
}
...
function openHelpDialog() {
  openModal(`
    <div class="modal-header"><div><h2>ショートカット</h2><div class="modal-subtitle">入力と構成をキーボードで操作できます。</div></div><button class="icon-button" data-modal-action="close">×</button></div>
    <dl class="help-list">
      <dt>段落内で改行</dt><dd><kbd>Enter</kbd></dd>
      <dt>次の段落へ分ける</dt><dd><kbd>Ctrl / ⌘</kbd> + <kbd>Enter</kbd></dd>
      ...
      <dt>メモを開閉</dt><dd><kbd>Ctrl / ⌘</kbd> + <kbd>Shift</kbd> + <kbd>M</kbd></dd>
      <dt>形式を選んでコピー</dt><dd><kbd>Ctrl / ⌘</kbd> + <kbd>Alt</kbd> + <kbd>C</kbd></dd>
      <dt>元に戻す / やり直す</dt><dd><kbd>Ctrl / ⌘</kbd> + <kbd>Z</kbd> / <kbd>Shift + Z</kbd></dd>
    </dl>
  `);
}
...
function openMoreDialog() {
  openModal(`
    ...
    <p class="privacy-note">myspines 0.4.0 ・ 原稿は外部へ送信されません。</p>
  `);
}
