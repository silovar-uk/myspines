    ...
    <div class="app-shell" data-mode="${mode}" data-outline-open="${outlineOpen}">
      <header class="global-header">
        <div class="global-header__left">
          ...
          ${mode === "write" ? `<button class="outline-open-button" data-action="toggle-outline" aria-pressed="${outlineOpen}"><span aria-hidden="true">☰</span> アウトライン</button>` : `<span class="global-document-name">${escapeHtml(state.book.title)}</span>`}
        </div>
        ...
        <div class="global-header__right">
          ...
          <button class="secondary-button header-action" data-action="margin">メモ${hasAnyNotes() ? '<span class="button-dot" aria-hidden="true"></span>' : ""}</button>
...
function renderWriteMode() {
  ...
  return `
    <div class="write-shell ${outlineOpen ? "is-outline-open" : "is-outline-closed"}" style="--outline-width:${outlineWidth}px">
      ...
      <aside class="outline-pane" aria-label="アウトライン">
        ${renderOutlinePanel()}
        <div class="outline-resizer" data-outline-resizer role="separator" tabindex="0" aria-label="アウトラインの幅" aria-orientation="vertical" aria-valuemin="220" aria-valuemax="400" aria-valuenow="${outlineWidth}"></div>
      </aside>
...
function renderOutlinePanel() {
  const metricsExpanded = state.book.view.outlineMetrics === true;
  return `
    <div class="outline-pane__header">
      <div>
        <h2>アウトライン</h2>
        <p>${state.book.manuscript.length}起点 ・ ${bookCharacters().toLocaleString("ja-JP")}字</p>
      </div>
      <div class="outline-pane__actions">
        <button class="metrics-toggle" data-action="toggle-outline-metrics" aria-pressed="${metricsExpanded}">文字数</button>
...
function renderSidebarNodes(nodes, depth, container) {
  ...
  return nodes.map((node) => {
    ...
    const showCount = state.book.view.outlineMetrics === true || depth === 0 || selected;
    ...
    return `
      <div class="sidebar-row ${selected ? "is-selected" : ""}" style="--depth:${depth}" data-node-id="${node.id}" role="treeitem">
        ...
        ${showCount ? `<span class="sidebar-count">${subtreeCharacters(node).toLocaleString("ja-JP")}</span>` : '<span class="sidebar-count is-hidden" aria-hidden="true"></span>'}
      </div>
      ${childRows}
    `;
  }).join("");
