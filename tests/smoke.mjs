...
const [html, base, editorCss, responsive, density, refinementsCss, materiality, core, editor, model, modelRefinements, uiShell, uiStructure, refinements, events, version, bootstrap, manifest] = await Promise.all([
  read("../index.html"),
  read("../v4/base.css"),
  read("../v4/editor.css"),
...
assert.match(html, /v4\/refinements\.css/u, "簡素化した表示を適用する");
assert.match(html, /v4\/materiality\.css/u, "紙とインクの質感を最後に適用する");
...
assert.match(core, /function ownNodeCharacters\(node\) \{\s*return countCharacters\(node\.body\);/u, "項目文字数は本文だけを数える");
assert.match(core, /function bookCharacters\(book = state\.book\)/u, "本文全体の文字数を独立して集計する");
...
assert.match(refinements, /title-character-count/u, "タイトル文字数を独立表示する");
assert.match(refinements, /heading-character-count/u, "各見出しの文字数を独立表示する");
assert.match(refinements, /body-character-count/u, "本文合計を本文として明示する");
assert.match(refinements, /target\.classList\.contains\("node-body"\)/u, "本文入力時だけ本文合計を更新する");
assert.match(refinements, /見出し番号/u, "本文上で見出し番号を切り替えられる");
assert.match(refinements, />操作<\/button>/u, "三点記号ではなく意味の読める操作ボタンを表示する");
assert.match(refinementsCss, /title-character-count/u, "タイトル文字数を控えめに整形する");
assert.match(refinementsCss, /heading-character-count/u, "見出し文字数を控えめに整形する");
...
assert.match(materiality, /--ink:\s*#24323a/u, "本文を青黒い万年筆インク色にする");
assert.match(materiality, /--brass:\s*#96753f/u, "真鍮を想起する控えめな補助色を定義する");
assert.match(materiality, /Yu Mincho/u, "タイトルと見出しに明朝系書体を使う");
assert.match(materiality, /manuscript-region::before/u, "原稿用紙の格子を余白へ描画する");
assert.match(materiality, /mask-image:\s*linear-gradient/u, "格子を本文の背後から避ける");
assert.match(materiality, /Keep the outline monochrome/u, "アウトラインを無彩色で維持する意図を残す");
assert.match(materiality, /forced-colors:\s*active/u, "高コントラスト表示では質感を取り除く");
assert.match(version, /myspines 0\.4\.4/u, "画面上のバージョンをpackageと一致させる");
...
console.log("myspines UI v4 character count smoke checks passed");
